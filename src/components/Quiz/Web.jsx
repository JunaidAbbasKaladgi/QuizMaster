...
const Web = () => {
    ...
    let [question, setQuestion] = useState(Webdev[index])
    ...
    const next = () => {
        if (index == Webdev.length - 1) {
            setResult(true)
        } else if (lock === true) {
            setIndex(++index)
            setQuestion(Webdev[index])
            setLock(false)
    ...
    return (
        <div>
            ...
            <div className='container'>
                ...
                <hr />
                {result ? <>
                    <h2>You scored {score} out of {Webdev.length}</h2>
                    ...
                    <div className="index">
                        {index + 1} of {Webdev.length} questions
                    </div>
                </>}
            </div>
